...
//         const response = await fetch(`${url}/daily/${userCountry.toLowerCase()}.json`);
//         const data = await response.json();
//         obj[userCountry] = {
//                 flagUrl: `https://countryflagsapi.com/svg/${userCountry.toLowerCase().slice(0, 2)}`,
//                 ...data,
//             };
//     });
...
//         const markup = `
//             <li class="flex"> 
//                 <h2 class="flag-bg" style="background-image: url('${obj[key].flagUrl}')">${key}</h2>
//                 <div class="converter__courses_value">
//                     <p>Sell ${obj[key][BASIC_COUNTRY.toLowerCase()].rate.toFixed(2)}</p>
//                     <p>Buy ${(obj[key][BASIC_COUNTRY.toLowerCase()].rate * BANK_COMMISSION).toFixed(2)}</p>
//                 </div>
//             </li> `
...
//             coefficient = obj[refs.selectFrom.value][refs.selectTo.value.toLowerCase()].rate;
            
//             if (obj[refs.selectFrom.value][refs.selectTo.value.toLowerCase()].rate < obj[refs.selectTo.value][refs.selectFrom.value.toLowerCase()].rate) {
...
//         if (refs.selectFrom.value !== refs.selectTo.value) {
//             coefficient = obj[refs.selectTo.value][refs.selectFrom.value.toLowerCase()].rate;
...
//     const calculateFromInput = () => refs.inputTo.value = (refs.inputFrom.value * (coefficient)).toFixed(2);
//     const calculateFromInput2 = () => refs.inputFrom.value = (refs.inputTo.value * (coefficient)).toFixed(2);
